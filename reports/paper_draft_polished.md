...
The paper proceeds as follows. Section 2 develops the theoretical framework, formalizing the thermostatic feedback model and introducing the augmented model with a frictional attenuation parameter (φ_F) that distinguishes two types of institutional delay. Section 3 describes the research design: the 51-state panel, the policy intensity index, the disaggregated backlash measures, and the estimation strategy. Section 4 presents the results, leading with the ESEA waiver finding as recommended by our peer review panel, and then interpreting the null baseline (H1) and the biennial legislature result (H2) as theoretically coherent predictions of the augmented model. Section 5 synthesizes the findings under the conditional feedback framework, discusses implications for the broader thermostatic literature, and acknowledges limitations. Section 6 concludes. Appendices present the formal ODE derivation with the φ_F extension, six state case studies organized by scope condition, and the full robustness table.
...
We estimate three primary specifications. The baseline fixed-effects specification regresses the backlash composite on lagged policy intensity with state and year fixed effects and standard errors clustered by state. The H7b interaction specifications add the product of backlash and active waiver status as predictors of first-differenced policy intensity (ΔP), with state and year fixed effects. The Granger causality test uses a Helmert-transformed GMM panel VAR estimating a two-variable system (policy intensity, backlash) where the Helmert transformation removes Nickell bias and transformed one-period lags are instrumented with untransformed lagged levels. The H2 biennial legislature test is a cross-sectional regression of the standard deviation of detrended policy intensity on a binary indicator for biennial legislative schedules.
...
We interpret the null theoretically as a consequence of high institutional friction that attenuates correction signals, while acknowledging that the district-level extension (N ≈ 13,000) will provide the statistical power needed to distinguish a true null from a small effect.
...
The lagged policy-norm gap itself is highly significant in predicting policy correction (β = 0.098, p = 0.000), but this likely reflects the arithmetic properties of the EWMA norm construction rather than behavioral evidence of policymaker responsiveness. We treat this result as mechanical rather than behavioral and do not claim it as evidence of thermostatic correction. Crucially, conditional on this mechanical control, lagged backlash does not significantly predict policy corrections in the expected direction (β = −0.037, p = 0.090).
...
This synthesis generates the paper's primary forward prediction: thermostatic feedback should be observable in precisely those institutional contexts where φ_F is low. The district-level extension provides a direct test: districts with no pre-existing teacher evaluation infrastructure that faced maximum new constraint from ESEA waivers ("newly constrained" districts) should show the strongest lock-in effects, while districts that already had strong evaluation systems ("always-compliant" districts) should show minimal constraint effects. This heterogeneity prediction follows directly from the φ_F formalization and is pre-specified.
...
The most important next step is the district-level extension. With approximately 13,000 districts and within-state variation in pre-waiver teacher evaluation infrastructure, the district design will provide both the statistical power to detect small thermostatic effects and the identification strategy — comparing "always-compliant" districts to "newly constrained" districts within the same state — to isolate the causal effect of the compliance commitment mechanism from selection into waiver adoption. The COVID-era parent rights movement offers a second test in a different institutional context: one where federal lock-in was absent, suggesting that thermostatic correction should have been substantially faster — the pinned pendulum released.
